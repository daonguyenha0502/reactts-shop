    ...
    return (
        <div className="w-4/5 lg:hidden flex-row h-auto bg-gray-500 rounded-b-lg absolute top-12">
            ...
            <ul className="mt-2 w-min mx-auto flex-row sm:flex-row md:flex-row lg:hidden">
                {!users.accessToken ? (<><li className="px-10 bg-blue-500 rounded-sm mb-2 text-black block sm:block md:block lg:hidden">
                    <Link className="cursor-pointer" to="/login"><span className={pathName.pathname === '/login' ? "text-red-700" : ""}>Login</span></Link>
                </li>
                    <li className="px-10 bg-blue-500 rounded-sm mb-2  text-black block sm:block md:block lg:hidden">
                        <Link className="cursor-pointer" to="/register"><span className={pathName.pathname === '/register' ? "text-red-700" : ""}>Register</span> </Link>
                    </li></>) : (<><li className="px-10 bg-blue-500 rounded-sm mb-2 text-black block sm:block md:block lg:hidden">
                        <Link className="cursor-pointer" to="/profile"><span className={pathName.pathname === '/profile' ? "text-red-700" : ""}>Profile</span></Link>
                    </li>
                        <li className="px-10 bg-blue-500 rounded-sm mb-2  text-black block sm:block md:block lg:hidden">
                            <Link className="cursor-pointer" onClick={() => handleLogOut()} to="/logout">Logout </Link>
                        </li></>)}
