...
const Checkout = ({ cartItems }: Props) => {
    ...
    return (
        <div className="w-92 sm:w-120 bg-indigo-400 py-4 rounded-md leading-8">
            ...
            {
                user.accessToken ? (<Link to="/checkout"><button className="bg-blue-600 px-5 py-1 rounded-md hover:bg-red-400 active:bg-red-500 duration-500 focus:outline-none">Check out</button></Link>) : (<Link to="/login"><button onClick={handleClick} className="bg-blue-600 px-5 py-1 rounded-md hover:bg-red-400 active:bg-red-500 duration-500 focus:outline-none">Check out</button></Link>)
            }
