    ...
    reducers: {
        ...
        reducerCart: (state, action: PayloadAction<itemType>) => {
            for (let i = 0; i < state.length; i++) {
                if (state[i]._id === action.payload._id && state[i].cartAmount === 1) {
                    return state.filter((product) => product._id !== action.payload._id)
                }
                else if (state[i]._id === action.payload._id && state[i].cartAmount > 1) {
                    ...
                    let a: any = state.find((product) => product._id === action.payload._id)
