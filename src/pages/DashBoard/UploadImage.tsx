        ...
        <>
            {role === 'admin' ? (
                <><Helmet>
                    ...
                    <div className="py-16 w-5/6 sm:w-5/6 md:w-5/6 lg:w-5/6 xl:w-5/6 2xl:w-3/4 mx-auto">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="flex-row md:flex w-auto h-auto justify-center items-center"><div><input className="border-2 focus:outline-none text-base mr-4" type="file" multiple accept="image/*" name="file" ref={register({ required: true })} /></div>
                                <div><input className="px-4 mt-4 md:mt-0 py-2 bg-blue-900 text-white rounded-md" type="submit" value="Upload" /></div>
                            </div>
                            {errors.file?.type === 'required' && (
                                <div><p className="text-base text-red-600 text-center">File is required</p></div>
                            )}
